...
player_dataset.fillna('N/A', inplace=True)
...
if 'Weekly Wages' in player_dataset.columns and 'Market Value' in player_dataset.columns:
    player_dataset['Weekly Wage'] = player_dataset['Weekly Wage'].apply(convert_wage)
    player_dataset['Market Value'] = player_dataset['Market Value'].apply(convert_wage)
else:
...
missing_columns = [col for col in required_columns if col not in player_dataset.columns]
if missing_columns:
    print(f"Error: The following required columns are missing from the dataset: {missing_columns}")
    # Handle or exit, as necessary
else:
    X = player_dataset[required_columns]

# **Identify the correct target variable column name. If it's not 'Future', replace it below.**
if 'YOUR_TARGET_COLUMN' in player_dataset.columns:  # Replace 'YOUR_TARGET_COLUMN'
    y = player_dataset['YOUR_TARGET_COLUMN']  # Replace 'YOUR_TARGET_COLUMN'
else:
    raise ValueError(f"The target column 'YOUR_TARGET_COLUMN' is missing from the dataset.") # Replace 'YOUR_TARGET_COLUMN'
...
player_dataset['Target'] = player_dataset.apply(lambda row: 1 if row['Performance Metric'] > threshold else 0, axis=1) # Define threshold logic

# Selecting features for the model
features = ['Performance Metric', 'Wages', 'Age', 'Position Code'] # Adjust features accordingly
X = player_dataset[features]
y = player_dataset['Target']
