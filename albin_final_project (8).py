...
if 'offensive_score' in player_dataset.columns and 'defensive_score' in player_dataset.columns and 'Weekly Wages' in player_dataset.columns and 'Age' in player_dataset.columns:
    offensive_threshold = np.percentile(player_dataset['offensive_score'], 70)
    defensive_threshold = np.percentile(player_dataset['defensive_score'], 70)
    wage_threshold = np.percentile(player_dataset['Weekly Wages'], 40)
    age_threshold = np.percentile(player_dataset['Age'], 60)

# Assigning 'look for better option' if a player is above thresholds for performance and below for wages
    ...
    # Assigning 'look for better option' based on available features
    player_dataset['future'] = np.where(
...
# Ensuring the 'future' column has both categories
...
# Checking new target distribution
...
# Encoding target variable after reassignment
if 'future' in player_dataset.columns:
    y = player_dataset['future'].apply(lambda x: 1 if x == 'look for better option' else 0)

# Feature Selection using Mutual Information
# Checking if 'Player' and 'future' columns exist before dropping them
...
X = player_dataset.drop(columns=columns_to_drop)
...
# Checking if there are any remaining NaN values in the dataset after imputation
if X.isna().sum().sum() > 0:
...
class CustomThresholdClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        self.thresholds_ = X.mean(axis=0) * self.threshold
        return self
...
# Initializing and train the custom model
...
# Model Interpretation using SHAP (for XGBoost only)
...
# Aligning the player column by index
player_column = player_dataset_copy['Player'].reset_index(drop=True)
player_dataset.insert(0, 'player', player_column)

predicted_columns= ['player', 'future', 'future_xgb', 'future_rf', 'future_custom']
final_player_dataset = player_dataset[predicted_columns]
...
final_player_dataset.to_excel(output_path, index=False)
